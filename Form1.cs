...
namespace Demo
{
    public partial class Form1 : Form
    {
        ...
        private void pictureBox1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            //  if(index==12)
            //{
            //    double angle = 2 * Math.PI / 3; // Góc giữa các cạnh của tam giác đều
            ...
            //        x[i] = cX + size * Math.Cos(i * angle);
            //        y[i] = cY + size * Math.Sin(i * angle);
            //        Point[] points = new Point[] { new Point((int)x[0], (int)y[0]), new Point((int)x[1], (int)y[1]), new Point((int)x[2], (int)y[2]) };
